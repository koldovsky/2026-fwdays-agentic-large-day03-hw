...
The system SHALL clear `selectedLinearElement` to null when Escape cancels an in-progress element creation. This ensures subsequent linear element creation starts from a clean state without inheriting stale editing context from the cancelled element.

#### Scenario: Line tool works after freedraw Escape cancel
- **WHEN** user selects the freedraw tool, starts drawing a stroke, presses Escape to cancel, then selects the line tool and clicks on canvas to start a line
...
- **WHEN** user selects the rectangle tool, starts dragging to create a rectangle, presses Escape to cancel, then selects the arrow tool and clicks on canvas
...
The system SHALL clear `startBoundElement` to null when Escape cancels an in-progress element creation. This prevents stale binding references from interfering with subsequent arrow/line creation.

#### Scenario: Arrow binding works after cancelled arrow
- **WHEN** user starts dragging an arrow from a shape, presses Escape to cancel, then starts a new arrow from the same or different shape
...
- **WHEN** user starts dragging to create a shape and presses Escape to cancel
...
The system SHALL trigger a scene re-render after soft-deleting the in-progress element during Escape cancellation. The element SHALL not remain visible on canvas after the Escape key is processed.

#### Scenario: Rectangle disappears on Escape
- **WHEN** user starts dragging to create a rectangle and presses Escape
...
- **WHEN** user starts drawing a freedraw stroke and presses Escape
