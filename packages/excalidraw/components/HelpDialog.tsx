...
const ShortcutIsland = ({
  ...
  if (query && filteredChildren.length === 0) {
    return null;
  }

  const childrenWithSearch = query
  ...
  return (
    <div className={`HelpDialog__island ${className ?? ""}`}>
      <h4 className="HelpDialog__island-title">{caption}</h4>
      <div className="HelpDialog__island-content">{childrenWithSearch}</div>
    </div>
  );
};
...
const Shortcut = ({
  ...
  const renderLabel = () => {
    ...
    return (
      <>
        {label.slice(0, idx)}
        <mark>{label.slice(idx, idx + searchQuery.length)}</mark>
...
export const HelpDialog = ({ onClose }: { onClose?: () => void }) => {
  ...
  const activateSearch = React.useCallback((initialChar?: string) => {
    ...
    <div className="HelpDialog__title-row">
      ...
      {searchActive ? (
        <input
          ...
          aria-label={t("helpDialog.searchPlaceholder")}
          placeholder={t("helpDialog.searchPlaceholder")}
        />
      ) : (
        <button
          className="HelpDialog__search-btn"
